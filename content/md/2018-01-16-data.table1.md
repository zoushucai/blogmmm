...
意思是: 对于数据集`DT`，选取子集行`i`, 通过`by`分组计算`j`。`i`设定数据的选取条件，`j`设定结果的计算方式，`by`设定数据的分组情况。通过这个，我们可以在一行代码中很方便地完成处理过程。首先需要把数据变为data.table类型
...
有！优化R的原生语法，使用内置的索引。这样我们毋需使用新的语法，就能得到同样的效果。
这就是自动索引。
...
## .2 data.table有一个**特殊的变量`.N`**可以直接计算分组的观测值个数。
